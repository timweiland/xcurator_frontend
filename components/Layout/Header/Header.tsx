...
const SearchBar = (): JSX.Element => (
  <div className="flex items-center text-gray-600 rounded-full bg-white shadow-xl">
    <input
      className="rounded-l-full w-full py-4 px-6 leading-tight text-sm focus:outline-none"
      type="search"
      name="search"
      placeholder="Search"
    />
...
const ProfileSection = (): JSX.Element => {
  ...
  if (!isAuthenticated || !user) {
    return (
      <NextLink href="/login" className="text-white font-bold">
        Login
      </NextLink>
    );
  }
                  ...
                  <Menu.Item key={userMenuItem.name}>
                    {({ active }) => (
                      ...
                      >
                        ...
                        <span className="ml-2">{userMenuItem.name}</span>
                      </NextLink>
                    )}
                  </Menu.Item>
                ))}
